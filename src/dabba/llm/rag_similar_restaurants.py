...
def _build_faiss_index(
    ...
    try:
        ...
        _faiss_index = index
        logger.info("FAISS index built with %d embeddings (dim=%d)",
                    len(embeddings), dim)
    except ImportError:
...
def build_restaurant_embeddings(
    ...
    logger.info("Saved restaurant embeddings to %s (shape=%s)",
                config.restaurant_embeddings_path, embeddings.shape)
...
def find_similar_restaurants(
    ...
    if index is not None:
        ...
        sim_scores = 1.0 / (1.0 + distances[0][1:top_k + 1])  # convert L2 to similarity
    else:
        # sklearn fallback
