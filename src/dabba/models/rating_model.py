...
def get_rating_models() -> Dict[str, Any]:
    """Return a dictionary of candidate rating prediction models.
...
def train_and_evaluate_rating_models(
    ...
    for name, model in models.items():
        ...
        logger.info(
            "%s — MAE: %.4f, RMSE: %.4f, R²: %.4f, Time: %.1fs",
...
def fit_best_rating_model(
    ...
    """Retrain the winning model on the full dataset and save to disk.
    ...
    Args:
        best_name: Name of the winning model (key from get_rating_models()).
    ...
    logger.info("Fitting best rating model '%s' on full data (%d samples)...", best_name, len(X))
    pipe.fit(X, y)

    save_model(pipe, save_path)
    logger.info("Best rating model '%s' fitted and saved to %s", best_name, save_path)
    return pipe
