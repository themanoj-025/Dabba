...
This module only defines the model dictionary (with optional neural
network) and thin wrappers.
...
def get_eta_models() -> Dict[str, Any]:
    """Return candidate ETA prediction models with default hyperparameters.
...
def get_tuned_eta_models(
    ...
    KNN, DecisionTree) keep their defaults since HPO gives marginal gains.
...
def _get_pytorch_nn(input_dim: int) -> Any:
    ...
    Uses PyTorch with a simple MLP architecture. Wrapped in a
    scikit-learn compatible interface via skorch if available,
    ...
    Returns:
        A skorch NeuralNetRegressor instance, or None if skorch/PyTorch
        is not installed.
    """
    try:
        import torch
...
def train_and_evaluate_eta_models(
    ...
    with the ETA-specific model dictionary and task name.

    Also attempts to train a small PyTorch MLP via skorch if the
...
def fit_best_eta_model(
    ...
    return _fit_best_model(
        best_name, X, y, models, save_path, task="eta", config=config
    )
