...
def clean_zomato(
    ...
    # Note: use column replacement (df[col] = ...) rather than in-place
    # df.loc[:, col] = ... — the cleaned float Series replaces the column
    # wholesale. In-place .loc assignment into an existing string-dtype column
    # raises TypeError on pandas >= 3 (strict string dtype); column replacement
    # yields identical results on pandas 2 and 3+.
